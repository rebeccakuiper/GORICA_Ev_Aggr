...
get_n_study_proportion <- function(n_studies){
  five_third <- round(n_studies / (5/3))
  one_third <- round((n_studies - five_third) / 2)
  rest <- n_studies - five_third - one_third
  return(list(five_third = five_third, one_third = one_third, rest = rest))
}
...
analysis <- function(set, n_studies, model_types, true_R2, 
                     beta_ratios, rho, N, inverse_ratio, outlier_study, specs, rownum){
  ...
  models <- sapply(1:n_studies, function(study){   # models used in different studies
    p = n.coef[study]
    eval(parse(text = specs$models[[model_types]][[study]]))
  })
  n_study_proportions <- get_n_study_proportion(n_studies)
  ...
  model_forms <- switch(model_types, 'hetero' = c(
    rep('probit', n_study_proportions[["one_third"]]),
    rep('linear', n_study_proportions[["five_third"]]), 
    rep('logit', n_study_proportions[["rest"]])
    ), 
    {rep('linear', n_studies)})
  
  # estimate parameters + vcov matrix
  intercept <- 0 # remains zero over all iterations as everything is standardized
  beta_ests <- lapply(1:n_studies, function(study){
    ...
    study_true_R2 <- true_R2
    study_true_ratio <- ratio
    if(outlier_study == "inverse" && study == n_studies){
      study_true_R2 <- 0.02
      study_true_ratio <- rev(ratio)
    }
    if(outlier_study == "extreme" && study == n_studies){
      study_true_R2 <- 0.25
      study_true_ratio <- rev(ratio)
    }
    estimate_betas(study_true_R2, study_true_ratio, mu, sigma, N, models[study], intercept, model_forms[study])
  })
  ...
  results <- GoricEvSynEssentials(S = n_studies, Param_studies, CovMx_studies, SameHypo = 1, NrHypos = n.hypos, Hypo_studies = hyp_set, Safeguard)
  results_added <- c(rownum, results$added[(n_studies+1),]) # n_studies+1 = because we need the final weights, which is (number of studies+1)th row. 
  results_equal <- c(rownum, results$equal[(n_studies+1),])
...
performance <- function(row){ # (row, threshold)
  ...
  if(set == 5 && row$inverse_ratio == 1){
    true_hyp = 3
  }
  if(set == 7 && row$inverse_ratio == 1){
    true_hyp = 2
  }
